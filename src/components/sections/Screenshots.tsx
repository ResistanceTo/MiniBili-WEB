import type { ScreenshotsProps, MediaItem } from "config";
...
const Screenshots = ({ images }: ScreenshotsProps) => {
	...
	return (
		<div className="mb-16">
			...
			<div className={`relative overflow-hidden min-h-[${isIphone ? "560px" : "300px"}]`}>
									...
									>
										{media.type === "video" ? (
											<div
												...
												tabIndex={0}
												role="button"
												aria-label={`${isPlaying ? '暂停' : '播放'} MiniBili ${activeDevice === "iphone" ? "iPhone" : "iPad"} 应用视频预览`}
											>
												<video
													...
													className={`rounded-xl border border-gray-300 dark:border-white/10 object-cover shadow-lg ${isIphone ? "aspect-[1170/2532] w-[260px]" : "aspect-[1488/2266] w-[340px]"
														}`}
												/>
												{/* 加载指示器 */}
												{isLoading && (
