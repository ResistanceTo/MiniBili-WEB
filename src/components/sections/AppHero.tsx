	...
	<div className="mb-16 flex flex-col items-center md:items-start md:flex-row gap-8">
		<div className="flex-shrink-0 md:self-center">
			<div className="rounded-2xl border border-gray-200/50 dark:border-white/10 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-white/[0.02] dark:to-white/[0.05] p-4 w-[192px] h-[192px] flex items-center justify-center shadow-sm">
				<img src={logo.src} alt="App Icon" className="w-full h-full rounded-2xl object-contain" />
			</div>
		</div>
		...
		<div className="flex flex-1 flex-col justify-between text-center md:text-left">
			...
			<div className="flex flex-wrap gap-4 justify-center md:justify-start">
				<StoreButton href={storeLinks.apple} label="Download on the" storeName="TestFlight" />
			</div>
		</div>
	</div>
);

const StoreButton = memo(({ href, label, storeName }: StoreButtonProps) => {
	const Icon = FaApple;
	...
	return (
		...
		>
			<div className="flex items-center justify-center w-7 h-7">
				<Icon className={`text-gray-600 dark:text-gray-300 transition-transform duration-300 group-hover:scale-110 group-hover:text-gray-800 dark:group-hover:text-white w-[22px] h-[22px]`} />
			</div>
